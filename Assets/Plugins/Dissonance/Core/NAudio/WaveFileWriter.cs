...
// ReSharper disable InconsistentNaming, FieldCanBeMadeReadOnly.Local, CheckNamespace
...
namespace NAudio.Wave
{
    ...
    internal class WaveFileWriter : Stream
    {
        ...
        private void UpdateFactChunk(BinaryWriter writer)
        {
            if (HasFactChunk())
            {
                int bitsPerSample = 32 * _format.Channels;//(format.BitsPerSample * format.Channels);
                if (bitsPerSample != 0)
                {
